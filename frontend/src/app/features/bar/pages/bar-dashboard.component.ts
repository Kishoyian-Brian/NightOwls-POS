...
export class BarDashboardComponent implements OnInit {
  ...
  loadOrders(): void {
    this.orders = this.orderService.getBarOrders();
  }
  ...
  getDrinkItems(order: Order): OrderItem[] {
    return this.orderService.getDrinkItems(order);
  }

  getDrinkTotal(order: Order): number {
    return this.orderService.calculateTotal(this.getDrinkItems(order));
  }
